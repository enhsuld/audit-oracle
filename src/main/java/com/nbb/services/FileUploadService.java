...
public class FileUploadService {
	...
	public LnkAuditFile handleNotlohZuil(MultipartFile mfile, String path, String downdir, String description,long appid) {
		String filename = mfile.getOriginalFilename();
		int index=filename.lastIndexOf('.');
		String lastOne=(filename.substring(index +1));
		System.out.println(lastOne);;
		...
		LnkAuditFile newFile = new LnkAuditFile();
		newFile.setFname(uuid);
		newFile.setFsize(size/1024);
		newFile.setFilename(filename);
		newFile.setMimetype(lastOne);
		newFile.setAppid(appid);
