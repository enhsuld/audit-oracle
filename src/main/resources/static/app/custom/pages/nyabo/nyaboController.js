	...
	.controller("nyaboCtrl", ['$rootScope', '$scope', '$timeout', 'user_data', 'mainService', 'sweet', '$state', 'Upload', 'fileUpload','__env',
		function ($rootScope, $scope, $timeout, user_data, mainService, sweet, $state, Upload, fileUpload,__env) {
			...
			$scope.download = function (id) {
				mainService.withdomain('get', __env.apiUrl() +'/api/v1/file/download/' + id).then(function (data) {
			...
			$scope.uploadNotlohZuil = function (file, y) {
				var xurl = "";
				if (y == 1) {
					xurl = __env.apiUrl() +'/api/v1/checker';
				}
				else if (y == 2) {
					xurl = __env.apiUrl() +'/api/v1/nyabo';
				}
				else if (y == 3) {
					xurl = __env.apiUrl() +'/api/v1/tsh/1';
				}
				else {
					xurl = __env.apiUrl() +'/api/v1/tsh/2';
				}
				Upload.upload({
					url: xurl,
			...
			var progressbar = $("#file_upload-progressbar"),
				...
				settings = {
					...
					action: __env.apiUrl() +'/api/v1/nyabo', // upload url
			...
			$scope.downloadExcel = function () {
				if (user_data.plan != "basic") {
					...
					mainService.withdomain('get', '/api/v1/file/verify/excel').then(function (response) {
						if (response.excel) {
							var link = document.createElement('a');
							link.href = __env.apiUrl() +'/api/v1/file/download/excel';
			...
			$scope.surveyExcel = function () {
				if (user_data.plan != "basic") {
					$rootScope.content_preloader_show();
					mainService.withdomain('get', __env.apiUrl() +'/api/v1/file/verify/excel').then(function (response) {
						if (response.excel) {
							var link = document.createElement('a');
							link.href = __env.apiUrl() +'/api/v1/file/download/survey';
							link.download = "Filename";
							link.click();
							setTimeout(function () {
								$rootScope.content_preloader_hide();
			...
			$scope.viewExcel = function () {
				...
				mainService.withdomain('get',__env.apiUrl() + '/api/v1/file/verify/excel').then(function (response) {
					...
					if (response.file || response.excel) {
						$scope.xlsx = true;
						$scope.purl = __env.apiUrl() +'/api/v1/file/sheet/excel';
