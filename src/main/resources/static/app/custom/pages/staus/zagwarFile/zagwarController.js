    ...
    .controller("fileCtrl", ['$scope', '$timeout', 'Upload', 'mainService', '$state', 'sweet',
        function ($scope, $timeout, Upload, mainService, $state, sweet) {
            ...
            $scope.download = function (id) {
                mainService.withdomain('get', '/api/files/' + id).then(function (data) {
            ...
            $scope.deleteO = function (item) {
                ...
                sweet.show({
                    ...
                    if (inputvalue) {
                        ...
                        mainService.withdata('POST', '/core/delete/' + $scope.domain, $scope.formdata).then(function (data) {
                            $(".k-grid").data("kendoGrid").dataSource.read();
        ...
        /*    $scope.submitUpload = function () {
            ...
            $scope.fileGrid = {
                ...
                columns: [
                    ...
                    {
                        field: "filename",
                        template: "<a  class='uk-text-primary uk-margin-remove' href='/api/files/1/#:name#.' target='_self' download='#:filename#'>#:filename#</a>",
                        title: "Үйлчлүүлэгчийн загвар"
                    },
                    {field: "filesize", title: "Хэмжээ", template: "<span>#:filesize# KB</span>"},
                    {
                        field: "filenameAdmin",
                        title: "Админий загвар",
                        template: "<a  class='uk-text-primary uk-margin-remove' href='/api/files/2/#:nameAdmin#.' target='_self' download='#:filenameAdmin#'>#:filenameAdmin#</a>"
                    },
