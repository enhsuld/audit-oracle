    ...
    .controller("majorCtrl", ['$rootScope', '$timeout', '$scope', 'cfpLoadingBar', 'user_data', 'mainService', 'sweet', '$state', '$stateParams', 'mainobj', 'Upload', 'fileUpload',
        function ($rootScope, $timeout, $scope, cfpLoadingBar, user_data, mainService, sweet, $state, $stateParams, mainobj, Upload, fileUpload) {
            ...
            $scope.reportChange = function () {
                ...
                mainService.withdata('post', '/api/work/' + mainobj.id, $scope.mainobj).then(function (data) {
            ...
            $scope.twit = {};
            var tw = UIkit.modal("#modal_twitter", {modal: false, keyboard: false, bgclose: false, center: false});

            $scope.twitts = function (i, y) {
                mainService.withdomain('get', '/api/twitter/' + i + '/' + y).then(function (data) {
                    $scope.comments = data;
                });
            };

            $scope.twitter = function (item) {
                $scope.twitts(item.formid, item.id);
                $scope.workTitle = item.data2;
                $scope.twit.appid = item.id;
                $scope.twit.formid = item.formid;
                $scope.twit.userid = user_data.id;
                tw.show();
            };

            $scope.submitTwitter = function () {
                mainService.withdata('post', '/api/twitter', $scope.twit).then(function (data) {
                    $scope.twit.comment = '';
                    $scope.twitts($scope.twit.formid, $scope.twit.appid);
                });
            };

            $scope.modalZegwarExcel = function () {
                mainService.withdomain('get', '/api/files/3/' + mainobj.orgtype).then(function (data) {
                    $scope.formdata = data[0];
                    modalUpdate.show();
                });
            };

            var modalNotloh = UIkit.modal("#modal_notloh_zuil", {
                modal: false,
                keyboard: false,
                bgclose: false,
                center: false
            ...
            $scope.selectJournal = function () {
                ...
                var modalPivot = UIkit.modal("#modal_full_pivot", {
                    modal: false,
                    keyboard: false,
                    bgclose: false,
                    center: false
            ...
            $scope.report = function () {
                mainService.withdomain('get', '/api/excel/report/' + mainobj.id).then(function (data) {
            ...
            $scope.fileGrid = {
                ...
                columns: [
                    ...
                    {
                        field: "filename",
                        template: "<a  class='uk-text-primary uk-margin-remove' href='/api/files/#:appid#/#:id#' target='_self' download='#:filename#'>#:filename#</a>",
                        title: "Файлын нэр"
                    },
                    ...
                    {
                        field: "comment",
                        title: "Аудиторийн санал",
                        editor: $scope.auditorEditor,
                        template: "<span class='uk-text-danger'>#:comment#</span>"
                    },
            ...
            var levels = [{"text": "Төлөвлөх", "value": 1}, {"text": "Гүйцэтгэх", "value": 3}, {
                "text": "Тайлагнах",
                "value": 4
            }, {"text": "Нотлох зүйл, Шинжилгээний горим", "value": 2}];
            ...
            $scope.mainGridOptions = {
                dataSource: {
                    ...
                    transport: {
                        read: {
                            ...
                            contentType: "application/json; charset=UTF-8",
                            data: {
                                "custom": "where appid=" + $stateParams.issueId + " and parentid is null",
                ...
                columns: [
                    ...
                    {field: "data2", title: "Чанарын баталгаажуулалтын асуулга"},
                    {
                        ...
                        width: 120,
                        title: " ",
                        editor: $scope.levelEditor,
                        template: "#if (levelid==1 && data13==2) {# <span>Төлөвлөх</span> #} else if(levelid==2 && data13==2){# <span>Гүйцэтгэх</span> #}#",
                        values: alevel
                    },
            ...
            var asd = [{"text": "Буцаасан", "value": 5}, {"text": "Хадгалсан", "value": 0}, {
                "text": "Илгээсэн",
                "value": 1
            }, {"text": "Хянасан", "value": 2}, {"text": "Баталгаажсан", "value": 3}];
            if (user_data.positionid === 1) {
                steps = [{"text": "Илгээх", "value": 1}];
                $scope.mainGridOptions.columns.push({
                    title: "Шийдвэр", width: 130, editor: function (container, options) {
                        ...
                        if (options.model.stepid === 0 || options.model.stepid === 5) {
                            ...
                            $("<strong>Боломжгүй</strong>").appendTo(container);
                        }
                ...
                $scope.mainGridOptions.columns.push({
                    title: "Төлөв",
                    width: 100,
                    template: kendo.template($("#step").html())
                });
                $scope.mainGridOptions.columns.push({
                    title: "Санал",
                    width: 100,
            ...
            $scope.ordersGridOptions = function (dataItem) {
                return {
                    dataSource: {
                        ...
                        transport: {
                            read: {
                                ...
                                contentType: "application/json; charset=UTF-8",
                                data: {
                                    "custom": "where appid=" + $stateParams.issueId + " and data8=1",
                    ...
                    columns: [
                        ...
                        {
                            field: "data4",
                            attributes: {"class": "customClassCenter"},
                            title: "Татуулах",
                            values: aj,
                            width: 132
                        },
                        ...
                        /*   { field: "data9", title:"Тайлбар", editor: $scope.categoryDropDownEditor, editable:false,width:300},*/
                        {
                            field: "data8",
                            title: "Маягт хавсрах",
                            template: kendo.template($("#uptemp").html()),
                            width: 160
                        },
                        {
                            field: "data7",
                            title: "Загвар татах / харах",
                            template: kendo.template($("#downtemp").html()),
                            width: 150
                        },
                ...
                $scope.ordersGridOptions.columns.push({
                    template: kendo.template($('#file').html()),
                    locked: true,
                    lockable: false,
                    width: 300
                })
            ...
            $scope.exportExcel7 = function (dataItem) {
                $rootScope.content_preloader_show();
                mainService.withdomain('get', '/api/excel/verify/nbb/' + dataItem.appid + '/' + dataItem.formid).then(function (response) {
                    if (response !== false) {
                        var link = document.createElement('a');
                        link.href = '/api/excel/export/form6/' + dataItem.appid + '/' + dataItem.formid;
                        link.download = "Filename";
                        link.click();
                        setTimeout(function () {
                            $rootScope.content_preloader_hide();
            ...
            var modalDownload = UIkit.modal("#modal_header_footer_download", {
                modal: false,
                keyboard: false,
                bgclose: false,
                center: false
            ...
            $scope.modalExcelDownload = function (dataItem) {
                mainService.withdomain('get', '/api/excel/report/' + mainobj.id + '/' + dataItem.data13).then(function (data) {
                    $scope.reportData = data;
                    if (data.length > 0) {
                        modalDownload.show();
                        progressbar.addClass("uk-hidden");
            ...
            var comType = [{"text": "Залруулах", "value": 1}, {
                "text": "Акт тавих",
                "value": 2
            }, {"text": "Албан шаардлага өгөх", "value": 3}, {"text": "Зөвлөмж өгөх", "value": 4}];

            var comResult = [{"text": "Залруулсан", "value": 1}, {
                "text": "Залруулаагүй",
                "value": 2
            }, {"text": "Зөвшөөрсөн", "value": 3}, {"text": "Тайлбартай", "value": 4}];

            var ismatter = [{"text": "Материаллаг", "value": 1}, {"text": "Материаллаг бус", "value": 0}];
            var yesNo = [{"text": "Тийм", "value": false}, {"text": "Үгүй", "value": true}];

            var lastlevel = [{"text": "Төлөвлөх", "value": 1}, {"text": "Гүйцэтгэх", "value": 2}, {
                "text": "Түүвэр",
                "value": 3
            }];

            var modalProb = UIkit.modal("#modal_problem_grid", {
                modal: false,
                keyboard: false,
                bgclose: false,
                center: false
                ...
                $scope.problemGridAkt = {
                    ...
                    columns: [
                        ...
                        },
                        {
                            field: "acc",
                            attributes: {"class": "customClassCenter"},
                            template: "<span>#=acc#</span><br><span>#=accCode#</span>",
                            title: "Сорил хийсэн дансны нэр",
                            width: 200
                        },
                        {
                            field: "problem",
                            ...
                            title: "Аудитаар илэрсэн асуудал",
                            template: "<span class='just'>#:problem#</span> <a href='/api/excel/export/akt/{{planid}}/#:id#'  target='_self' download='асуудлын бүртгэл'  class='md-btn md-btn-mini md-btn-wave'><i class='uk-icon-download'></i></a>",
                            width: 200
                        },
                        {
                            field: "amount",
			                          ...
			                          { field:"comAmount", title:"Тухайн байгууллагын саналын  дүн", width:200},*/
                        {
                            field: "comMatter",
                            values: ismatter,
                            title: "Санхүүгийн тайланд үзүүлэх нөлөө (материаллаг, материаллаг бус)",
                            width: 200
                        },
                        {
                            field: "comAktName",
                            attributes: {"class": "customClass"},
                            editor: $scope.textEditor,
                            title: "Зөрчсөн эрхийн актын нэр",
                            width: 200
                        },
                        {
                            field: "comAktZaalt",
                            attributes: {"class": "customClass"},
                            editor: $scope.textEditor,
                            title: "Зөрчсөн эрхийн актын заалт",
                            width: 200
                        },
                        {field: "stepid", title: "Үе шат", hidden: true, values: lastlevel, width: 200},
                        {field: "finish", title: "Буцаах эсэх", values: yesNo, width: 200},
                        {
                            field: "finalAktAmount",
                            attributes: {"class": "customClassCenter"},
                            template: "<span>#:finalAktAmount# ₮</span>",
                            title: "Аудитаар эцэслэсэн дүн",
                            width: 200
                        },
                        {
                            template: "<a href='/api/excel/export/akt/{{planid}}/#:id#'  target='_self' download='асуудлын бүртгэл'  class='k-button k-button-icontext'><i class='uk-icon-download'></i> Татах</a>",
                            width: 100
                        },
                    ],
                    editable: true,
                ...
                $scope.problemGridAsh = {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            attributes: {"class": "customClass"},
                            title: "Аудитаар илэрсэн асуудал",
                            template: "<span class='just'>#:problem#</span> <a href='/api/excel/export/ash/{{planid}}/#:id#'  target='_self' download='асуудлын бүртгэл'  class='md-btn md-btn-mini md-btn-wave'><i class='uk-icon-download'></i></a>",
                            width: 200
                        },
                        {
                            field: "amount",
                        ...
                        {field: "finish", title: "Буцаах эсэх", values: yesNo, width: 200},
                        {
                            field: "finalAshAmount",
                            attributes: {"class": "customClassCenter"},
                            template: "<span>#:finalAshAmount# ₮</span>",
                            title: "Аудитаар эцэслэсэн дүн",
                            width: 200
                        },
                        {
                            template: "<a href='/api/excel/export/ash/{{planid}}/#:id#'  target='_self' download='асуудлын бүртгэл'  class='k-button k-button-icontext'><i class='uk-icon-download'></i> Татах</a>",
                            width: 100
                        },
                    ],
                    editable: true,
            ...
            $scope.modalZuvlumjView = function (dataItem) {
                ...
                $scope.problemGridZuv = {
                    ...
                    columns: [
                        ...
                        {field: "finish", title: "Буцаах эсэх", values: yesNo, width: 200},
                        {
                            field: "finalZuvAmount",
                            attributes: {"class": "customClassCenter"},
                            template: "<span>#:finalZuvAmount# ₮</span>",
                            title: "Аудитаар эцэслэсэн дүн",
                            width: 200
                        },
            ...
            $scope.modalProblem = function (dataItem) {
                ...
                $scope.problem = true;
                $scope.advice = false;
                mainService.withdomain('get', '/api/excel/problem/' + mainobj.id).then(function (data) {
                    $rootScope.content_preloader_hide();
                    modalProb.show();
                    $scope.fin = true;
                    $scope.domainProblem = "com.nbb.models.fn.LnkAuditProblem.";
                    ...
                    $scope.problemGrid = {
                        ...
                        columns: [
                            ...
                            },
                            {
                                field: "commentType",
                                values: comType,
                                title: "Аудиторын санал (бүртгэлийн бичилт, тооцоолол)",
                                width: 200
                            },
                            {
                                field: "aktName",
                            ...
                            },
                            {
                                field: "comResult",
                                values: comResult,
                                title: "Тухайн байгууллагын зөвшөөрсөн эсэх",
                                width: 200
                            },
                            {field: "comAmount", title: "Тухайн байгууллагын саналын  дүн", width: 200},
                            {
                            ...
                            },
                            {
                                field: "result",
                                values: comType,
                                title: "Уулзалтын үр дүнгийн баталгаажуулалт",
                                width: 200
                            },
                            {
                                field: "comAktName",
            ...
            $scope.submitUploadOffer = function () {
                if ($scope.formFile.afl.$valid && $scope.afl) {
                    bar.css("width", "0%").text("0%");
                    progressbar.removeClass("uk-hidden");
                    $scope.upload($scope.afl, 4);
                }
            }
            ...
            $scope.upload = function (file, i) {
                ...
                if (i == 0) {
                    ...
                    xurl = '/api/excel/upload/offer/' + $stateParams.issueId;
                } else {
                ...
                Upload.upload({
                    ...
                    if (resp.data.excel) {
                        ...
                        UIkit.notify("Excel загвар тохирохгүй байна. " + resp.data.sheetname + "", {
                            status: 'error',
                            pos: 'top-right'
                        });
                        $scope.sendBtn = true;
                        ...
                        if (resp.data.support) {
                            ...
                            if (resp.data != null) {
                                ...
                                angular.forEach($scope.forms, function (value, key) {
                                    UIkit.notify(value.fname + " маягтыг амжилттай хавсарлаа.", {
                                        status: 'success',
                                        pos: 'top-right'
                                    });
                                });
